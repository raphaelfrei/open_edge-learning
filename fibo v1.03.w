...
------------------------------------------------------------------------------*/
    DO WITH FRAME {&FRAME-NAME}:
        ...
        /* Define o corpo da STRING por cada vez que o LOOP é rodado */
        texto = xd + "FIB[" + STRING(xc + 2) + "] = " + 
            STRING(INT(xa + xb)) + ",".
        
        /* Controla a recursividade da função, definindo novos valores
        de INPUT */
        IF xc < INT(sld-1:SCREEN-VALUE) - 2 THEN

            RUN Fibo(xb, INT(xa + xb), INT(xc + 1), texto).
            
        /* Quando a operação chega ao fim, roda os seguintes comandos
            e encerra o LOOP */
        ELSE DO:
